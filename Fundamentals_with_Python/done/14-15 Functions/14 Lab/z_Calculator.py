"""
    ...
    You select operation from the list, then select two int /float numbers and get result of the function operation.
    ...
    while True:
        ...
        if command == str(len(menu) - 1):
            ...
            if not command.isdigit():
                ...
                if len(menu) - 1 <= command_int or command_int < 1:
                    print(f"\nSelect valid Option from Menu, in range [1,{len(menu) - 1}] ")
                    continue
                operation = menu[command_int]
                while True:
                    number_1 = input("Enter the first number: ")
                    if not (all(char.isdigit() or char == '.' or (char == '-' and i == 0) for i, char in
                                enumerate(number_1.strip()))):
